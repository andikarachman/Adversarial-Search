...
class CustomPlayer(DataPlayer):
    ...
    **********************************************************************
    ...
    def get_action(self, state, depth_max=100):
        ...
        **********************************************************************
        ...
        for dp in range(1, depth_max+1):
            self.queue.put(self.minimax(state, depth=dp))
    ...
    # Penalize corner heuristics
    def score(self, state):
        own_loc = state.locs[self.player_id]
        opp_loc = state.locs[1 - self.player_id]
        own_liberties = state.liberties(own_loc)
        opp_liberties = state.liberties(opp_loc)
        corners = [0, 10, 114, 104]
        corner_weight = 2
        number_own_moves = len(own_liberties)
        number_opp_moves = len(opp_liberties)
        if own_loc in corners:
            number_own_moves -= corner_weight
        if opp_loc in corners:
            number_opp_moves -= corner_weight

        return number_own_moves - number_opp_moves
